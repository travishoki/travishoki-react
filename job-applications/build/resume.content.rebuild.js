// Tailored ATS resume content for the Re:Build (Reflow) Senior Frontend
// Engineer application. One minimal, honest edit on top of the master
// resume.content.js: the summary leads with the literal AI-tooling match
// (Cursor AI, Claude Code), Storybook/component-driven experience, Vite
// and Cypress from the current stack, the structured-AI-output pattern
// from Food Cards, and the 0-to-1 startup pattern from Spiff, all named
// directly in this posting. Job history, bullets, projects, skills, and
// education are unchanged from the master.
...
module.exports = {
	...
	summary:
		'Senior Software Engineer with 14+ years of experience building, scaling, and modernizing web applications. Frontend specialist in React and TypeScript who uses Cursor AI and Claude Code daily in production development. I have real, extensive experience with Storybook and component-driven development, and my current stack includes Vite and Cypress. I have also shipped a structured-AI-output feature in production: my Food Cards project resolves natural-language queries into schema-validated filters, so an ambiguous model response degrades gracefully instead of breaking the UI. I have the 0-to-1 startup pattern this kind of role calls for: I joined Spiff as an early engineer, built platform infrastructure from scratch, and stayed through its acquisition by Salesforce.',
